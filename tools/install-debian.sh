...
#
# Bootstrap rhorizon on a fresh Debian 12 (bookworm) system.
...
# PostgreSQL 18 from the signed PGDG repo (Debian's default is older). The
# repo is GPG-signed and we pin its key with signed-by, so apt still verifies
# package signatures -- PG18 parity without weakening integrity.
. /etc/os-release
...
# python3 is 3.11 on Debian 12 ; rhorizon supports 3.11+ and the test suite
# passes on 3.11 as well as 3.12. python3-venv is split out. libsodium-dev for
# pynacl, libldap2-dev/libsasl2-dev for bonsai, libffi-dev for cffi,
# build-essential for the C extensions. We do NOT install Debian's cargo/rustc
# (bookworm ships rust 1.63, too old for Cargo.lock v4); rustup below pulls a
...
echo ">> installing rustup (Debian's apt cargo is rust 1.63 - too old for Cargo.lock v4)"
