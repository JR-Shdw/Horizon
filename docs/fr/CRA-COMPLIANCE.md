...
| (3) | Tests + revues de securite reguliers et efficaces | **CONFORME** | Plus de 3 000 tests Python et 272 tests Rust en mode release, `test_security.py` dedie, cargo-fuzz (4 cibles), miri, clippy `-D warnings` ; CI a chaque push, couverture 94%. La couverture fonctionnelle et KAT crypto s'exécute sur les deux architectures livrées : amd64 (`validate.yml`) et aarch64 (`arch-matrix.yml` / `tools/test-arm64.sh`). Une Action GitHub native contrôle aussi l'assembleur GF(256) aarch64 et exécute les quatre cibles de fuzzing sur ARM. |
