...
| medium | `/oneshot` non-constant-time `master_check` comparison | **fixed** | `api/app/routes/oneshot.py:183` uses `_hmac.compare_digest` |
...
| low | Shamir reconstruction left sub-keys in the Python heap | **fixed** | `api/app/cluster_setup.py` calls `secure_zero` on every key and on `secret_bytes` in a `finally` |
...
| low | `agent/rust/src/{inject,fetch,watch}.rs` | `secret_name` is concatenated into the URL without percent-encoding. Bearer auth still enforces RBAC. **Fixed** in `e607483`, across six sites |
...
12 findings: 0 critical, 2 high, 2 medium, 2 low, 6 info. **As of 2026-08-22 all six
non-info findings are fixed.** See the status table at the top of this file.
